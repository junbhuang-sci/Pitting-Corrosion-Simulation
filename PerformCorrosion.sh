# Perform corrosion on a given sample
# by Zhiliang Pan, 2022-5-24
#-------------------------------------------------------------
...
PotStyle=`grep pair_style $InputFile`
PairCoeff=`grep pair_coeff $InputFile`
...
NumNode=`head -2 $RunMpiFile|tail -1|awk -F= '{print $2}'`
NumCore=`head -3 $RunMpiFile|tail -1|awk -F= '{print $2}'`
...
$HOME/Fortran/CubePerformCorrosion \$DmpFile $Cutoff $ECut $Temperature
echo
